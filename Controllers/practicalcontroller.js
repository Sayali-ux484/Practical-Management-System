...
export const getPracticals = async (req, res) => {
    try {
        const practicals = await Practical.find()
            .populate("subjectId", "name code") // Populate subject name and code
            .populate("createdBy", "name email role") // Populate teacher details
            .populate("enrolledStudents", "name email"); // Populate student details
